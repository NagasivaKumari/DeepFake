...
const fetchTrustScore = async () => {
  const response = await fetch(`/media/trust?sha256_hash=example_hash`);
  if (!response.ok) {
...
export default function Dashboard() {
  ...
  const { data: trustScore, isLoading, error } = useQuery(["trustScore"], fetchTrustScore);
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-32">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg md:text-xl text-white/80 mb-6">
              Trust Score: {isLoading ? "Loading..." : error ? "Error" : `${trustScore}/5`}
            </p>
          ...
          <CardContent className="p-12 text-center relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to={createPageUrl("DeveloperDocs")}>
                <Button
                  size="lg"
                  className="bg-white text-blue-600 hover:bg-gray-100 shadow-lg gap-2"
                >
